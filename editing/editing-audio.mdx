...
These are the two types of edits you can make:

- A **cut** removes a portion of audio from playback. The surrounding audio plays continuously with no gap.
- A **mute** silences a portion of audio. The duration is preserved but the sound is replaced with silence.

You can undo any edit with Ctrl/Cmd+Z.
...
**Select tool (S)** is for making cuts and mutes. Click and drag on the waveform to select a region, then click **Cut** or **Mute** (or use keyboard shortcuts). This is the default mode.

**Move tool (V)** is for syncing multi-track recordings. When you have two or more audio files in the same block (e.g., separate microphones for host and guest), use the Move tool to shift one track's position relative to the other. This is how you align tracks that started recording at slightly different times.
...
If your audio has been transcribed, you can edit by reading and selecting text instead. Transcription happens automatically after upload, or you can trigger it manually by clicking **Transcribe** on an audio file.

To edit using the transcript:

1. Open the transcript panel for an audio block
2. Read through the text and find what you want to remove
...
Below the transcript you'll also find:

- **Find Filler Words** scans the transcript for "um," "uh," "like," and other filler words so you can quickly review and cut them.
