...
- **Left panel:** shows your episode title, status, and duration at the top. Below that are two tabs: **Files** (your uploaded audio files) and **Properties** (section and block settings). You drag files from here into audio blocks.
- **Main area:** the vertical stack of sections and blocks. Each section has a header with its name and BGM assignment. Inside, you see audio blocks with waveforms and pause blocks with their duration.
- **Bottom bar:** playback controls (play, rewind, fast-forward) and a minimap showing all sections so you can jump between them.

The top-right corner has **Save as Template** and **Export** buttons.
